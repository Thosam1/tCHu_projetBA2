            ...
            switch (turnKind) {
                ...
                case CLAIM_ROUTE:
                    ...
                    if (desiredRoute.level() == Route.Level.UNDERGROUND) {
                        ...
                        Game.infoToAll(players, currInf.attemptsTunnelClaim(desiredRoute, initialCards));
                        ...
                        int additionalCardsCount = desiredRoute.additionalClaimCardsCount(initialCards, drawnCards);
                        ...
                        if ((additionalCardsCount != 0) && (!possibleAdditionalCards.isEmpty())) {
                            ...
                            SortedBag<Card> cardsUsedWithAddition = initialCards.union(additionalCards);
                            ...
                            if (additionalCards == null || additionalCards.size() == 0) {
                                Game.infoToAll(players, currInf.didNotClaimRoute(desiredRoute));
                            }

                            //info la route a été prise par le joueur

                            else Game.infoToAll(players, currInf.claimedRoute(desiredRoute, cardsUsedWithAddition));
                            ...
                            gameState = (additionalCards == null || additionalCards.size() == 0) ? gameState :
                                    gameState.withClaimedRoute(desiredRoute, cardsUsedWithAddition);
                        }

                        //si le joueur n a pas de cartes additionnelles à poser alors il s'empare de la route
                        else if (additionalCardsCount == 0) {
                            Game.infoToAll(players, currInf.claimedRoute(desiredRoute, initialCards)); //info la route a été prise par le joueur
                            gameState = gameState.withClaimedRoute(desiredRoute, initialCards);
