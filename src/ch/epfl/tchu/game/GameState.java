...
public final class GameState extends PublicGameState{
    ...
    public GameState withChosenAdditionalTickets(SortedBag<Ticket> drawnTickets, SortedBag<Ticket> chosenTickets) { // faudrai-il utiliser currentPlayerId() au lieu de currentPlayerId ? oui en général avec les opérations
        ...
        playerState2.put(currentPlayerId, playerState.get(currentPlayerId).withAddedTickets(chosenTickets)); // ignorer le reste

        return new GameState(tickets.withoutTopCards(drawnTickets.size()), cardState, currentPlayerId, playerState2, lastPlayer); // Que fait-on des cartes non choisies ? On les ignore
    }
