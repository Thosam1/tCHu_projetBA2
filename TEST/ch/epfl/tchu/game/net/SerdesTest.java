...
public class SerdesTest {
    ...
    void TestNullValuesDeserialize() {
        assertEquals(null, Serdes.SERDE_PLAYER_ID.deserialize(""));
        assertEquals(null, Serdes.SERDE_ROUTE.deserialize(""));
        assertEquals(null, Serdes.SERDE_CARD.deserialize(""));
        assertEquals(null, Serdes.SERDE_TURN_KIND.deserialize(""));
        assertEquals(null, Serdes.SERDE_TICKET.deserialize(""));

        // TEST
        assertEquals("", Serdes.SERDE_LIST_OF_CARD.serialize(List.of()));
        // assertEquals(0, "".split(Pattern.quote(" "), -1).length);
    }

    @Test
    void TestNullValuesSerialize() {
        assertEquals("", Serdes.SERDE_PLAYER_ID.serialize(null));
        assertEquals("", Serdes.SERDE_ROUTE.serialize(null));
        assertEquals("", Serdes.SERDE_CARD.serialize(null));
        assertEquals("", Serdes.SERDE_TURN_KIND.serialize(null));
        assertEquals("", Serdes.SERDE_TICKET.serialize(null));

    }

    @Test
    void PublicCardStateSerialize1() {
        List<Card> fu = List.of(Card.of(Color.RED), Card.of(Color.WHITE),
                Card.of(Color.BLUE), Card.of(Color.BLACK), Card.of(Color.RED));
        PublicCardState cs = new PublicCardState(fu, 30, 31);
        String encoded = "6,7,2,0,6;30;31";
        assertEquals(Serdes.SERDE_PUBLIC_CARD_STATE.serialize(cs), encoded);
    }

    @Test
    void PublicCardStateDeserialize1() {
        String encoded = "6,7,2,0,6;30;31";
        PublicCardState test = Serdes.SERDE_PUBLIC_CARD_STATE
                .deserialize(encoded);
    ...
    void PublicCardStateSerialize2() {
        PublicCardState cardStateTest1 = new PublicCardState(
                SortedBag.of(5, Card.BLUE).toList(), 0, 0);
        PublicCardState cardStateTest2 = new PublicCardState(
                SortedBag.of(5, Card.BLUE).toList(), 5, 0);
        PublicCardState cardStateTest3 = new PublicCardState(
                SortedBag.of(5, Card.BLUE).toList(), 0, 5);

        String encoded1 = "2,2,2,2,2;0;0";
        String encoded2 = "2,2,2,2,2;5;0";
        String encoded3 = "2,2,2,2,2;0;5";

        assertEquals(Serdes.SERDE_PUBLIC_CARD_STATE.serialize(cardStateTest1),
                encoded1);
        assertEquals(Serdes.SERDE_PUBLIC_CARD_STATE.serialize(cardStateTest2),
                encoded2);
        assertEquals(Serdes.SERDE_PUBLIC_CARD_STATE.serialize(cardStateTest3),
                encoded3);
    }
    ...
    void PublicCardStateDeserialize2() {
        ...
        PublicCardState test1 = Serdes.SERDE_PUBLIC_CARD_STATE
                .deserialize(encoded1);
        PublicCardState test2 = Serdes.SERDE_PUBLIC_CARD_STATE
                .deserialize(encoded2);
        PublicCardState test3 = Serdes.SERDE_PUBLIC_CARD_STATE
                .deserialize(encoded3);
    ...
    void PublicPlayerStateSerialize1() {
        ...
        // String encoded2 = "20;21"; //TODO Ici, check comment le codé sera
        // avec une liste vide en dernier - ou rajouter ; à la fin ???
        String encoded2 = "20;21;";
        assertEquals(encoded1,
                Serdes.SERDE_PUBLIC_PLAYER_STATE.serialize(test1));
        assertEquals(encoded2,
                Serdes.SERDE_PUBLIC_PLAYER_STATE.serialize(test2));
    }
    ...
    void PublicPlayerStateDeserialize1() {
        String encoded1 = "10;11;0,1";
        // String encoded2 = "20;21"; //TODO Ici, check comment le codé sera
        // avec une liste vide en dernier
        String encoded2 = "20;21;"; // il faut mettre un ; a la fin meme si la
                                    // liste est vide
        // ne pas mettre d'espace après le dernier ";"

        PublicPlayerState test1 = Serdes.SERDE_PUBLIC_PLAYER_STATE
                .deserialize(encoded1);
        PublicPlayerState test2 = Serdes.SERDE_PUBLIC_PLAYER_STATE
                .deserialize(encoded2);
        ...
        assertEquals(test2.routes().size(), 0); // comment faire la diff entre
                                                // un 0 et un null ??? en bas
                                                // pour lastPlayer null etc.. ou
                                                // liste vide ou pas de route

    }

    @Test
    void PublicPlayerStateSerialize2() {
        String encoded1 = "1;1;15"; // on fait séparément et pas la map toute
                                    // seule ici
        ...
        PublicPlayerState test1 = new PublicPlayerState(ticketCountPlayer1,
                cardCountPlayer1, listeDeRoutes1);
        PublicPlayerState test2 = new PublicPlayerState(ticketCountPlayer2,
                cardCountPlayer2, listeDeRoutes2);
    ...
    void PublicPlayerStateDeserialize2() {
        ...
        assertEquals(ChMap.routes().get(15).id(), test1.routes().get(0).id());// on
                                                                              // compare
                                                                              // les
                                                                              // ids
                                                                              // sinon
        ...
        assertEquals(ChMap.routes().get(10).id(), test2.routes().get(0).id());// on
                                                                              // compare
                                                                              // les
                                                                              // ids
                                                                              // sinon
    ...
    void PlayerStateSerialize() {
        ...
        assertEquals(encoded1, Serdes.SERDE_PLAYER_STATE.serialize(test));
    }
    ...
    void PlayerStateDeserialize() {
        ...
        cardList.add(Card.of(Color.BLACK)); // 1 black
        cardList.add(Card.of(null)); // locomotive
        SortedBag<Card> cards = SortedBag.of(cardList); // -> in order so 0 2
                                                        // 333 8
        ...
        PlayerState test = Serdes.SERDE_PLAYER_STATE.deserialize(encoded1);
    ...
    void PublicGameStateTestSerialise() {
        ...
        PublicCardState cs = new PublicCardState(fu, 30, 31);
        List<Route> rs1 = ChMap.routes().subList(0, 2);
        Map<PlayerId, PublicPlayerState> ps = Map.of(PlayerId.PLAYER_1,
                new PublicPlayerState(10, 11, rs1), PlayerId.PLAYER_2,
                new PublicPlayerState(20, 21, List.of()));
        PublicGameState gs = new PublicGameState(40, cs, PlayerId.PLAYER_2, ps,
                null);

        assertEquals(Serdes.SERDE_PUBLIC_GAME_STATE.serialize(gs),
    ...
    void PublicGameStateTestDeserialise() {
        ...
        PublicGameState test = Serdes.SERDE_PUBLIC_GAME_STATE
                .deserialize(encoded);
        ...
        assertTrue(
                test.cardState().faceUpCards().contains(Card.of(Color.BLACK)));
    ...
    void PublicGameStateTestSerialise2() {
        Map<PlayerId, PublicPlayerState> validPlayerState = new EnumMap<>(
                PlayerId.class);
        int ticketCountPlayer1 = 1;
        int ticketCountPlayer2 = 2;
        int cardCountPlayer1 = 1;
        ...
        PublicPlayerState playerState1 = new PublicPlayerState(
                ticketCountPlayer1, cardCountPlayer1, listeDeRoutes1);
        PublicPlayerState playerState2 = new PublicPlayerState(
                ticketCountPlayer2, cardCountPlayer2, listeDeRoutes2);
        ...
        PublicGameState test = new PublicGameState(0, cardStateTest1,
                PlayerId.PLAYER_1, validPlayerState, PlayerId.PLAYER_1);
        PublicGameState test2 = new PublicGameState(5, cardStateTest2,
                PlayerId.PLAYER_1, validPlayerState, null);
        PublicGameState test3 = new PublicGameState(1, cardStateTest3,
                PlayerId.PLAYER_2, validPlayerState, null);

        String encodedTest = "0:2,2,2,2,2;0;0:0:1;1;15:2;2;10,29:0";
        String encodedTest2 = "5:2,2,2,2,2;5;0:0:1;1;15:2;2;10,29:";
        // String encodedTest3 = "1:2,2,2,2,2;0;5:1:1;1;15:2;2;10,29"; //Est ce
        // que tu as fait expres de ne pas mettre : à la fin
        String encodedTest3 = "1:2,2,2,2,2;0;5:1:1;1;15:2;2;10,29:";
        assertEquals(encodedTest,
                Serdes.SERDE_PUBLIC_GAME_STATE.serialize(test));
        assertEquals(encodedTest2,
                Serdes.SERDE_PUBLIC_GAME_STATE.serialize(test2));
        assertEquals(encodedTest3,
                Serdes.SERDE_PUBLIC_GAME_STATE.serialize(test3));
    }
    ...
    void PublicGameStateTestDeserialise2() {
        String encodedTest = "0:2,2,2,2,2;0;0:0:1;1;15:2;2;10,29:0";
        String encodedTest2 = "5:2,2,2,2,2;5;0:0:1;1;15:2;2;10,29:";
        // String encodedTest3 = "1:2,2,2,2,2;0;5:1:1;1;15:2;2;10,29"; //il faut
        // mettre : à la fin
        String encodedTest3 = "1:2,2,2,2,2;0;5:1:1;1;15:2;2;10,29:";

        PublicGameState test = Serdes.SERDE_PUBLIC_GAME_STATE
                .deserialize(encodedTest);
        PublicGameState test2 = Serdes.SERDE_PUBLIC_GAME_STATE
                .deserialize(encodedTest2);
        PublicGameState test3 = Serdes.SERDE_PUBLIC_GAME_STATE
                .deserialize(encodedTest3);

        assertEquals(test.ticketsCount(), 0);

        assertEquals(test.cardState().faceUpCards().size(), 5);
        assertTrue(
                test.cardState().faceUpCards().contains(Card.of(Color.BLUE)));
        assertTrue(
                !test.cardState().faceUpCards().contains(Card.of(Color.GREEN)));
        ...
        assertTrue(
                test2.cardState().faceUpCards().contains(Card.of(Color.BLUE)));
        assertTrue(!test2.cardState().faceUpCards()
                .contains(Card.of(Color.GREEN)));
        ...
        assertTrue(
                test3.cardState().faceUpCards().contains(Card.of(Color.BLUE)));
        assertTrue(!test3.cardState().faceUpCards()
                .contains(Card.of(Color.GREEN)));
